...
{//вызывается при некоторых событиях или начальной иницилизации
	DrawNode::onEnter();
	
	
	//// タッチ関係のイベント管理 - управлялка touch событиями
	//touchMgr=new TouchManager();
	
	// デバイス座標からスクリーン座標に変換するための - преопразовывает координаты устройства в экранные
	//deviceToScreen=new L2DMatrix44();
	
	// 画面の表示の拡大縮小や移動の変換を行う行列 .. Матрица, которая преобразует отображение на экране
	...
	//												 // Диапазон экрана, соответствующий устройству. Левый X, правый X, нижний Y, верхний Y
	//
	//
	...
	//
	//// 表示範囲の設定 // Настройка диапазона отображения
	//viewMatrix->setMaxScale( VIEW_MAX_SCALE );// 限界拡大率 // предел увеличения
	//viewMatrix->setMinScale( VIEW_MIN_SCALE );// 限界縮小率 //предел уменьшения
	//
	//// 表示できる最大範囲// максимальный диапазон, который может быть отображен
	...
	////// イベントリスナー作成 // Создать прослушиватель событий
 //   auto listener = EventListenerTouchAllAtOnce::create();
	//
 //   // タッチメソッド設定// Настройка сенсорного метода
 //   listener->onTouchesBegan = CC_CALLBACK_2(LAppView::onTouchesBegan, this);
 //   listener->onTouchesMoved = CC_CALLBACK_2(LAppView::onTouchesMoved, this);
 //   listener->onTouchesEnded = CC_CALLBACK_2(LAppView::onTouchesEnded, this);
 //  
	//
 //   // 優先度100でディスパッチャーに登録// Зарегистрироваться диспетчером с приоритетом 100
 //   this->getEventDispatcher()->addEventListenerWithFixedPriority(listener, 100);
...
{//отрисовка
	...
	_customCommand.func = CC_CALLBACK_0(LAppView::onDraw, this, transform, flags);//после говорим вызови ondraw
...
{
	kmGLPushMatrix(); // ??? добавляем матрицу
    kmGLLoadMatrix(&transform);// ??? грузим в матрицу
	
	LAppLive2DManager* Live2DMgr=LAppLive2DManager::getInstance();//получаем объект
	Live2DMgr->onUpdate();//обновляем
	
	
	kmGLPopMatrix();//?? удаляем матрицу
}
