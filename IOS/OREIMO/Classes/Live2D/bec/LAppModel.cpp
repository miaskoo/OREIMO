...
void LAppModel::update()
{
	...
	if (expressionMgr != NULL)
			expressionMgr->updateParam(live2DModel);// 表情でパラメータ更新（相対変化） // Parameter update with relative expression (relative change)

	if(physics!=NULL)physics->updateParam(live2DModel);// 物理演算でパラメータ更新 // Update parameter in physical operation
	
	// リップシンクの設定
	if(lipSync)
	{
		float value = 0;// リアルタイムでリップシンクを行う場合、システムから音量を取得して0～1の範囲で入力してください。 / When performing lip sync in real time, get the volume from the system and enter in the range of 0 to 1.
		live2DModel->setParamFloat(PARAM_MOUTH_OPEN_Y, value, 0.8f);
	}
	
	// ポーズの設定 // setting of pose
...
void LAppModel::setExpression(const char expressionID[])
{
	...
	loadExpression(name, (*dir_expression[expressionID]).c_str());
