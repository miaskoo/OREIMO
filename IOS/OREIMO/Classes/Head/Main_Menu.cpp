...
string dir_home_voice = ".\\Resources\\Voice";
#else
...
struct data_obj
{
	...
	std::string data1 = "";// переписываю и уподобляю информацию в один байт
	std::string data2 = "";
	std::string data3 = "";
	bool hide = false;
...
void init_creator_mode(Node *Creator_Mode)
{
	...
	Text_Node = Game_Mode->getChildByTag(1257);
	Text_Box = Text_Node->getChildByTag(4535);
	...
	Label_text = static_cast<ui::Text*>(Text_Node->getChildByTag(4534));
	Label_name = static_cast<ui::Text*>(Text_Node->getChildByTag(4533));
...
void MAINBOX::creatorset()
{
	...
	static const int predel_choise = 17;
	static Node *choises[predel_choise]//ноды выборов и настроек
	{
		Creator_Mode->getChildByTag(90), // choise type BOX (sprite, scene, text.....)
			...
			Creator_Mode->getChildByTag(1276)//WORLD ACTION - состояния рендера
	};
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	static ui::Button *type_obj_choise[]//тип объекта
	{
		static_cast<ui::Button*>(choises[0]->getChildByTag(1)), //SCENE
			...
			static_cast<ui::Button*>(choises[0]->getChildByTag(9)) // WORLD
	};
	...
	//WORLD
	//ACTION
	//просто напихал рендер состояния для сцен и спрайтов

	static ui::Button *wolrd_renderState_choise[] =
	{
		static_cast<ui::Button*>(choises[16]->getChildByTag(1)),//morning 
		static_cast<ui::Button*>(choises[16]->getChildByTag(2)),// evening
		static_cast<ui::Button*>(choises[16]->getChildByTag(3)),// night
		static_cast<ui::Button*>(choises[16]->getChildByTag(4))// sepia
	...
	auto update_inf([=]() //обновляет все везде где можно
	{
		...
		if (cur_page_count != NULL) // если есть страничка
		{
			...
			if (creator_obj != nullptr)
			{
					...
					else if (creator_obj->my_type == type_struct::sprite && creator_obj->my_inter == swich)
					{
						...
						if (creator_obj->my_name != any_enum::NONE)
						{
							...
							if (creator_obj->hide == false)
							{
								...
								{
									if (creator_obj->data1.length() != 0 && creator_obj->data2.length() != 0)
									{
										if (SPRITE_DIR[cur_capter].find(creator_obj->data1) != SPRITE_DIR[cur_capter].cend())
										{
											if (SPRITE_DIR[cur_capter][creator_obj->data1][NAME_MAP_SPRITE].find(creator_obj->data2) != SPRITE_DIR[cur_capter][creator_obj->data1][NAME_MAP_SPRITE].cend())
											{
												...
												if (creator_obj->data3.length() != 0 &&//не появилось при возврате к обжу... глянь потом в отладчике
				...
				else if (creator_obj->my_type == type_struct::world)
				{
					choises[16]->setVisible(true);
				}
			}
	...
	LOAD_ALL_PAGE->addClickEventListener([=](Ref*)
	{
		if (lock_creator_action == false)
		{
			...
			LOAD_ALL_PAGE->setTouchEnabled(false);
				if (load_script_exept() == true)
				{
					load_update();				
				}	
			lock_creator_action = false;
			LOAD_ALL_PAGE->setTouchEnabled(true);
		}
	});
	...
	type_obj_choise[8]->addClickEventListener([=](Ref*)
	{
		if (lock_creator_action == false)
		{
			if (creator_obj->my_type != type_struct::world)
			{
				*creator_obj = data_obj{};
				creator_obj->my_inter = action;
				creator_obj->my_type = type_struct::world;
				update_inf();
			}
		}
	});
	...
	for (int n = 0; n < 4; n++)
	{
		wolrd_renderState_choise[n]->addClickEventListener([=](Ref*)
		{
			creator_obj->data1 = std::to_string(n);
		});
	...
	auto create_page([=]() 
	{
		...
		static any_enum::global_filtr cur_state = any_enum::morning;
			...
			for (int i = 0; i < gpi()->cur_row_count[n]; i++)
			{
				if (sign(n, i)->my_type != type_struct::none)
				{
					...
					case type_struct::world:
					{
						...
						case action:
						{
							if (sign(n, i)->data1.length() > 0)
							{
								auto x = string_to_int(sign(n, i)->data1);
								switch (x)
								{
								case 0:
									cur_state = any_enum::morning;
									break;
								case 1:
									cur_state = any_enum::evening;
									break;
								case 2:
									cur_state = any_enum::night;
									break;
								case 3:
									cur_state = any_enum::sepia;
									break;
								default:
									break;
								}
					...
					case type_struct::scene:
					{
						...
						case swich:
						{
							objs.push_back(new Object_Scene(sign(n, i)->data1, string_to_int(sign(n, i)->data2), cur_state));
						...
						case swich:
						{
							...
							else
							{
								auto sprite = new Object_Sprite(
									...
									string_to_int(sign(n, i)->data2), string_to_int(sign(n, i)->data3), cur_state);
					...
					case type_struct::text:
					{
						...
						case swich:
						{
							auto data = new Object_Text(sign(n, i)->data1, sign(n, i)->data2, sign(n,i)->data3);
							objs.push_back(data);
							objs_type.push_back(sign(n, i)->my_type);
							intrs.push_back(sign(n, i)->my_inter);
							swichs.push_back(sign(n, i)->my_swich);
...
void sound_play(string dir)
{
	...
	Game_id = cocos2d::experimental::AudioEngine::play2d(dir, false, All_Volume*Game_Volume);
}

void voice_play(string dir)
{
	if (dir == "")
	{
		return;
	...
	if (Voice_id != -1)
		cocos2d::experimental::AudioEngine::stop(Voice_id);
	Voice_id = cocos2d::experimental::AudioEngine::play2d(dir_home_voice+ "\\mp3\\0000" + dir+".mp3", false, All_Volume*Voice_Volume);
}
...
int save_script()
{
	...
	buff.push_back((unsigned char)cur_page_count);//записали количество страничек скрипта
...
void clear_mem_game()
{
	//если решили загружать - отчищаем загруженные данные (потом бы засунуть в функцию)
...
//записываю все данные, не парюсь о мусоре
...
int load_script(std::string custom_dir)
{
	...
	//открыли файлик, начали загрузку
	//отчиститли всю память от старого скрипта
	clear_mem_game();
	...
	auto Rb([&]()->int {//read bite
		char temp;
		file.read(&temp, 1);
		cur_bite = (unsigned char)temp;
		if (file.eof())
		{
			return 1;
	...
	R(Rb);
	int size_page = (int)cur_bite;//получили размер странички скрипта
	if (size_page < 1)
		return 1;
	page_obj cur_load_page;
	for (int p = 0; p < size_page; p++)
	{
		cur_load_page = page_obj{};
		R(Rb);
		int size_box = (int)cur_bite;
		for (int b = 0; b < size_box; b++)
		{
			R(Rb);
			int size_row = (int)cur_bite;
			for (int r = 0; r < size_row; r++)
			{
				R(Rb);
				int size_obj = (int)cur_bite;
				size_obj *= 255;
				R(Rb);
				size_obj += cur_bite;
				int size_data1 = 0;
				...
				int size_data3 = 0;
				int pre_bite = 15;
				int bite_data1 = pre_bite;
				int bite_data2 = pre_bite;
				int bite_data3 = pre_bite;
				
				//int cur_bite_num = 0;

				data_obj cur_load_obj;
				//скипаем первый байт тк уже считали
				for (int n = 2; n < size_obj; n++)
				{
					switch (n)
					{
					case 2:
					...
					{
						R(Rb);
						size_data2 = (int)cur_bite;
						size_data2 *= 255;
						bite_data2 = pre_bite + size_data1;
						break;
					}
					case 5:
					{
					...
					{
						R(Rb);
						size_data3 = (int)cur_bite;
						size_data3 *= 255;

						bite_data3 = size_data2 + bite_data2 ;
						break;
					}
					case 7:
					{
					...
					{
						R(Rb);
						cur_load_obj.my_type = static_cast<type_struct::type_object>(cur_bite);
						break;
					}
					case 9:
					{
						R(Rb);
						cur_load_obj.my_inter = static_cast<type_interactions>(cur_bite);
						break;
					}
					case 10:
					{
						R(Rb);
						cur_load_obj.my_swich = static_cast<type_swich>(cur_bite);
						break;
					}
					case 11:
					{
						R(Rb);
						cur_load_obj.my_action = static_cast<type_action>(cur_bite);
						break;
					}
					case 12:
					{
						R(Rb);
						cur_load_obj.my_name = static_cast<any_enum::name_sprite>(cur_bite);
						break;
					}
					case 13:
					{
						R(Rb);
						cur_load_obj.my_position = static_cast<any_enum::posicion_sprite>(cur_bite);
						break;
					}
					case 14:
					{
						R(Rb);
						cur_load_obj.hide = static_cast<bool>(cur_bite);
						break;
					}
					default:
					{
						if (n == bite_data1 && size_data1 > 0)
						{
							for (int z = 0; z < size_data1; z++)
							{
								R(Rb);
								cur_load_obj.data1 += cur_bite;
								if (z + 1 < size_data1)
									n++;
							}
							//n += size_data1 - 1;
						}						
						else if (n == bite_data2 && size_data2 > 0)
						{
							for (int z = 0; z < size_data2; z++)
							{
								R(Rb);
								cur_load_obj.data2 += cur_bite;
								if (z + 1 < size_data2)
									n++;
							}
						}
						else if (n == bite_data3 && size_data3 > 0)
						{
							for (int z = 0; z < size_data3; z++)
							{
								R(Rb);
								cur_load_obj.data3 += cur_bite;
								if (z + 1 < size_data3)
									n++;
							}
						}
				...
				cur_load_page.intedex_data[cur_load_page.cur_box_count][cur_load_page.cur_row_count[cur_load_page.cur_box_count]] = cur_load_obj;
		...
		cur_page_data[p] = new page_obj (cur_load_page);
		cur_page_count++;
	}
