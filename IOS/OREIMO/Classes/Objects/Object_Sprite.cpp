...
//довольно костыльное решение но переделывать лень
...
Object_Sprite::Object_Sprite(any_enum::name_sprite chapter_name, string type_sprite, int num_sprite, int num_face, any_enum::global_filtr render_type) : Object_Scene()// сгенерить данные спрайта и его координаты
{
	...
	if (this->dir_obj != "empty")
	{
		for (int n = 0; n < cout_sprite; n++)
		{
			if (check_sprite_for_state[index_sprite[n]] == any_enum::FREE && (*index_sprite[n])->getOpacity() == 0)
			{
				...
				//смена рендера прорисовки ипользуетс€ только в SWITCH !!!
				...
				//теперь сам слежу за выключением функции моргани€, работает фоном всегда до выключени€ + пам€ть нужно чистить самому
