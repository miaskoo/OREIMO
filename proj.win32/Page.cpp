...
bool Page::draw()
{
	...
	case type_struct::sprite:
	{
				   if (objects_intreractions[objects[cur_obj]] == action)
				   {
					   ...
					   {
						   auto cur_scale_chapter = &name_for_scale[static_cast<Object_Sprtite*>(objects[cur_obj])->Get_name()];
						   ...
						   {
							   auto cur_scale_sprite = &sprite_for_scale[name_for_node[static_cast<Object_Sprtite*>(objects[cur_obj])->Get_name()]];
				   ...
				   else if (objects_intreractions[objects[cur_obj]] == swich)
				   {
					   ...
					   {
						   if (sprite_for_scale[new_scale_node] != cur_scale_chapter && static_cast<Object_Sprtite*>(objects[cur_obj])->Get_hide() != true)
						   {
							   switch (cur_scale_chapter)
							   {
							   case any_enum::little:
								   data.new_ptr->setScale(1.7f);
								   *cur_scale_sprite = any_enum::little;
								   break;
							   case any_enum::standart:
								   data.new_ptr->setScale(1.9f);
								   *cur_scale_sprite = any_enum::standart;
								   break;
							   case any_enum::big:
								   data.new_ptr->setScale(2.1f);
								   *cur_scale_sprite = any_enum::big;
								   break;
							   default:
								   break;
					   ...
					   Action_sprite::swich(data.old_ptr, data.new_ptr, objects_swich[objects[cur_obj - 1]], static_cast<Object_Sprtite*>(objects[cur_obj - 1])->Get_hide(), objects[cur_obj - 1]->Get_wait(), this);
				   }
				   break;
	}
	...
	case type_struct::condition:
	{
								   if (objects_intreractions[objects[cur_obj]] == action)
								   {
									   ...
									   if (data.old_ptr == nullptr)
									   {
										   while (objects[cur_obj]->Get_wait() != true)
											   cur_obj++;
										   cur_obj++;
									   }
									   else 
